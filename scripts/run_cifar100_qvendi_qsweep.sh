#!/bin/bash
#SBATCH --job-name=cifar100_qsweep
#SBATCH --output=../logs/cifar100_qvendi_q_sweep_%j_%a.out
#SBATCH --error=../logs/cifar100_qvendi_q_sweep_%j_%a.err
#SBATCH --time=4:00:00
#SBATCH --partition=cs
#SBATCH --gres=gpu:1
...
# q-sweep for cifar-100: test q values [0.1, 0.5, 1.0, 2.0, 10.0, inf] + loss_diff baseline
...
if [[ "$Q_VALUE" == "baseline" ]]; then
    ...
    echo "=========================================="
    echo "running cifar-100 loss_diff baseline"
    ...
    echo "=========================================="
    echo "running cifar-100 q-vendi with q=${Q_VALUE}"
    echo "array task id: $SLURM_ARRAY_TASK_ID"
    echo "job id: $SLURM_JOB_ID"
    echo "=========================================="
fi
...
local_path="./results/split_cifar100_${METHOD_NAME}/test1"
...
if [[ "$Q_VALUE" == "baseline" ]]; then
    echo "completed cifar-100 loss_diff baseline"
else
    echo "completed cifar-100 q-vendi with q=${Q_VALUE}"
fi
