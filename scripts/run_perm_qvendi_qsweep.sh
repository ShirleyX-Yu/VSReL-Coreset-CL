#!/bin/bash
#SBATCH --job-name=perm_qsweep
#SBATCH --output=../logs/perm_qvendi_q_sweep_%j_%a.out
#SBATCH --error=../logs/perm_qvendi_q_sweep_%j_%a.err
#SBATCH --time=1:00:00
#SBATCH --partition=cs
#SBATCH --gres=gpu:1
...
# q-sweep for permuted mnist: test q values [0.1, 0.5, 1.0, 2.0, 10.0, inf] + loss_diff baseline
...
if [[ "$Q_VALUE" == "baseline" ]]; then
    ...
    echo "=========================================="
    echo "running permuted mnist loss_diff baseline"
    ...
    echo "=========================================="
    echo "running permuted mnist q-vendi with q=${Q_VALUE}"
    echo "array task id: $SLURM_ARRAY_TASK_ID"
    echo "job id: $SLURM_JOB_ID"
    echo "=========================================="
fi
...
local_path="./results/perm_mnist_${METHOD_NAME}/test1"
...
if [[ "$Q_VALUE" == "baseline" ]]; then
    echo "completed permuted mnist loss_diff baseline"
else
    echo "completed permuted mnist q-vendi with q=${Q_VALUE}"
fi
