#!/bin/bash
# run CIFAR PRV with loss_diff baseline (no Q-Vendi)

export CUBLAS_WORKSPACE_CONFIG=:4096:8

# parameters matching run_cifar_prv.sh
dataset='splitcifar'
setting='greedy'
data_path=''
buffer_size=200
...
local_path='./results/split_cifar_prv_loss_diff/test1'

# create logs directory if it doesn't exist
mkdir -p logs

# redirect all output to log file
exec > logs/cifar_prv_loss_diff.out 2>&1
