#!/bin/bash
#SBATCH --job-name=mnist_prv_qvendi_q_sweep
#SBATCH --output=logs/mnist_prv_qvendi_q_sweep_%A_%a.out
#SBATCH --error=logs/mnist_prv_qvendi_q_sweep_%A_%a.err
#SBATCH --array=0-5
...
ref_train_lr=3e-3
cur_train_lr=2e-2
ref_sample_per_task=5

local_path="./results/split_mnist_prv_qvendi_q${q}/seed${seed}"

echo "Running MNIST PRV with q=$q on GPU $CUDA_VISIBLE_DEVICES"
