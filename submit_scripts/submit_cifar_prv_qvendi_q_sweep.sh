#!/bin/bash
#SBATCH --job-name=cifar_prv_qvendi_q_sweep
#SBATCH --output=../logs/cifar_prv_qvendi_q_sweep_%A_%a.out
#SBATCH --error=../logs/cifar_prv_qvendi_q_sweep_%A_%a.err
#SBATCH --array=0-5
...
local_path="./results/split_cifar_prv_qvendi_q${q}/seed${seed}"

echo "Running CIFAR PRV with q=$q on GPU $CUDA_VISIBLE_DEVICES"
